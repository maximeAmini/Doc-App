...
export default {
    ...
    methods: {
        getNbPatient() {
            axios.get(route("patients.nb")).then((res) => {
                this.patients = res.data
            });
        },
        getNbRdv() {
            axios.get(route("rdvs.nb")).then((res) => {
                this.rdvs = res.data
            });
        },
        getNbReceptionniste() {
    ...
    <AuthenticatedLayout>
        <div class="flex align-center">
            <div>
                ...
                <h4 class="text-gray-400">Vous êtes connecté en tant que Receptionniste </h4>
            </div>
        </div>
        ...
        <div class="py-12 grid grid-cols-1 sm:grid-cols-3 sm:gap-4">
            <div
                ...
                <div class="text-gray-400">
                    Patients
                    <p class="font-bold text-lg text-gray-800">{{ patients }}</p>
                </div>
            ...
            <div
                class="border border-b-[#643A6A] border-b-4 shadow-gray-200 rounded-lg p-4 flex items-center hover:scale-105 transition delay-150 duration-300 ease-in-out mb-2 cursor-pointer">
                <div class="flex items-center justify-center mr-2 rounded-full bg-[#643A6A] text-white w-10 h-10">
                    <i class="fa-solid fa-calendar-days"></i>
                </div>

                <div class="text-gray-400">
                    Rdvs
                    <p class="font-bold text-lg text-gray-800">{{ rdvs }}</p>
                </div>

                <div class="w-14 h-14 flex justify-center items-center font-bold text-red-500 ml-auto">
                    -1
                    <i class="ml-1 fa-solid fa-down-long"></i>
                </div>
            </div>
            <div
