...
export default {
    ...
    methods: {
        deletemedecin(id) {
            Inertia.delete(route("medecins.destroy", id));
            this.medecins2 = this.medecins2.filter((obj) => obj.id !== id);
        },
        checkAll() {
            if (this.checkedAll == false) {
                var table = []
                this.medecins2.forEach(function (arrayItem) {
        ...
        deleteAll() {
            for (var i = 0; i < this.checkGroupe.length; i++) {
                this.deletemedecin(this.checkGroupe[i]);
            }
        },
        orgnise(type) {
            axios.get(route("medecins.orgnise", type)).then((res) => {
                this.medecins2 = res.data;
                if (this.type == "DESC") {
                    this.type = "ASC";
                } else {
                    this.type = "DESC";
        ...
        searchFunc() {
            axios.get(route("medecins.search", this.searchInput)).then((res) => {
                if (res.data != '') {
                    this.medecins2 = res.data;
                } else {
                    this.medecins2 = this.medecins
                }
                console.log(this.medecins2)
            });
        },
    },
    mounted() {
        ...
        <AuthenticatedLayout>
            ...
            <div class="flex align-center">
                ...
                <div class="ml-auto align-center">
                    ...
                    <Link :href="route('medecins.create')" alt="Add new medecin"
                    ...
                    <span class="hidden sm:inline">Ajouter un medecin</span>
                    </Link>
                </div>
            </div>
            ...
            <div class="text-gray-600 w-full mt-6" v-if="search">
                ...
                <input type="text" name="serch" placeholder="Search medecin..."
            ...
            <div class="py-4 sm:py-6">
                ...
                <nav aria-label="Breadcrumb" class="text-sm font-semibold mb-8">
                    <ol class="list-none p-0 inline-flex items-center ">
                        ...
                        <li class="flex items-center">
                            <Link :href="route('medecins.index')" alt="medecins" class="text-gray-300 font-bold">
                            Medecins</Link>
                        </li>
                    </ol>
                </nav>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">Nombre de consultations</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="bg-white border-b hover:bg-gray-50"
                                v-for="medecin in medecins2" :key="medecin.id">
                                <td class="w-4 p-4">
                                    <div class="flex items-center">
                                        <input id="checkbox-table-1" type="checkbox"
                                            class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500"
                                            :value="medecin.id" v-model="checkGroupe" />
                                    </div>
                                </td>
                                <th scope="row"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <span class="mr-2 font-bold">{{medecin.civilite}}</span>{{ medecin.user.name }}
                                    <div class="mt-2">
                                        <i @click="deletemedecin(medecin.id)"
